...
public class TriangleOverworldFactory {
	...
	private static int defineMode() {
		...
		return JOptionPane.showOptionDialog(null, "Random map or load map?", "TLT", JOptionPane.YES_NO_CANCEL_OPTION,
	...
	private static AbstractField[][] generateRandomMapMatrixOf(int worldSize) {
		AbstractField[][] field = IntStream.range(0, worldSize)
				.mapToObj(unusedBuffer -> generateRandomMapVectorOf(worldSize)).collect(Collectors.toList())
				.toArray(new AbstractField[0][0]);
		return field;
	}

	private static AbstractField[] generateRandomMapVectorOf(int worldSize) {
		return IntStream.range(0, worldSize).mapToObj(unusedBuffer -> FieldType.getRandomFieldType(random))
				.map(FieldType::newInstance).collect(Collectors.toList()).toArray(new AbstractField[0]);
	}
	 ...
	 */
	private static AbstractField[][] convertStringMapToFieldMap(TriangleOverworld triangleOverworld,
			List<String[]> mapData) {
